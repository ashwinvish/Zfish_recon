    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues+xOffset,m,'ok','MarkerEdgeColor','k','MarkerFaceColor','w','MarkerSize',7);
            end
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues+xOffset,md,'^r','MarkerEdgeColor','k','MarkerFaceColor','w','MarkerSize',7);
            end
            if opt.showMM == 4
